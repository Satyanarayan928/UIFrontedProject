...
function AllProductAdmin() {
    ...
    <Grid container rowSpacing={1}>
      ...
      {shoeData.map((data, index) => (
        <Card key={index} style={{ marginBottom: '16px' }}>
          <CardContent onClick={handleCardChange}>
            <img src={shoewhite}  width={100} height={100}/>
          ...
          <td>
          <img src={edit}  width={30} height={30} onClick={(e) => {
                e.preventDefault();
                const datas = { id: data.id, name: data.name, category: data.category, price: data.price, description: data.description, manufacturer: data.manufacturer, availableItems: data.availableItems };

                console.log("onclick data:", datas);

                localStorage.setItem('datas',JSON.stringify(datas));
                navigate("/modifyProductAdmin/${id}");
          }}/>
          </td>
          <td>
          <img src={dit}  width={30} height={30} onClick={(e) => {
             e.preventDefault();
             const id=data.id;
             shoeData(l => l.filter(item => item.id !== id));
          }}/>
          </td>
          </tr>
          </CardContent>
    ...
    <div>
      {bagData.map((data, index) => (
        <Card key={index} style={{ marginBottom: '16px' }}>
          ...
          <td>
          <img src={dit}  width={30} height={30}/>
          </td>
          </tr>
          </CardContent>
        </Card>
    ...
    <Grid size={30}>
    <div>
      {earBud.map((data, index) => (
        <Card key={index} style={{ marginBottom: '16px' }}>
          <CardContent>
            <img src={airpod_1}  width={100} height={100}/>
            <Typography variant="h6" align="right">{data.price}</Typography>
            <Typography variant="h6">{data.name}</Typography>
            <Typography variant="body2" color="textSecondary">
              {data.description}
