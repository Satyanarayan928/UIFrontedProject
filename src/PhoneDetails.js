 ...
 function PhoneDetails() {
    const navigate=useNavigate();
     ...
     const [productSelectPage, setProductSelectPage] = useState(null);
     ...
     const handleBagCardClick = (index) => {
           alert("bag:"+index);
     }

     const handleButtonClick = (selectPhoneData) =>{
         console.log("Phone Data:", selectPhoneData); 
         console.log("Phone ID:", selectPhoneData.id); 
         var id=selectPhoneData.id;
         console.log("Phone ID1:", id); 
         navigate(`/phoneDetailsPage/${id}`); 
     ...
     <Grid size={30} onClick={handleCellClick}>
     <div>
      {phone.map((data, index) => (
        <Card key={index} style={{ marginBottom: '16px' }}>
          <CardContent>
            ...
            <tr>
            <td>
          <Button type="submit" fullWidth variant="contained" color="primary" sx={{ mt: 3, mb: 2 }} onClick={()=>{
